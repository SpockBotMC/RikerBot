...
StatusCore::StatusCore(PluginLoader& ploader, bool ownership) :
  ...
  ev->register_callback("ClientboundPosition",
      [&](ev_id_type id, const void* p) {handle_ppl(id, p);});

  ev->register_callback("tick",
      [&](ev_id_type id, const void* p) {handle_tick(id, p);});
}

void StatusCore::handle_spawn(ev_id_type ev_id, const void* data) {
  ev->emit(status_spawn);
  ev->unregister_callback(ev_id, spawn_cb);
}

void StatusCore::handle_ppl(ev_id_type ev_id, const void* data) {
...
void StatusCore::handle_tick(ev_id_type ev_id, const void* data) {
